...
function generate() {
  ...
  let finalBlock =
    filteredBlocks[Math.floor(Math.random() * filteredBlocks.length)];
...
function isGameOver() {
  for (let i = 0; i < 16; i++) {
    ...
    if (currentTile === 0) {
      ...
      i % 4 !== 3 &&
      currentTile ===
        parseInt(document.querySelector(`#id_${i + 1}`).textContent)
    ) {
      return false;
    } else if (
      i < 12 &&
      currentTile ===
        parseInt(document.querySelector(`#id_${i + 4}`).textContent)
    ) {
      return false;
    }
  }
